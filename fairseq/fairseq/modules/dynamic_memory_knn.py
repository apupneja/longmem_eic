...
class Hierarchical_Memory:
    ...
    def __init__(self, cfg):
        ...
        self.assignments = []
        self.cluster_idx = 0
        self.num_cluster_knn = 15
        self.val_to_cluster_idx = {}
    ...
    def add_index(self, qkv_val, retrieval_layer_index=None, padding_mask=None):
        ...
        for i, index in enumerate(self.index_list):
            start = time.time()
            centroids, assignments = kmeans_cuda(keys_with_chunk[:, :, i, :].mean(dim=-2).cpu().numpy(), self.num_cluster_knn, seed=3)
            end = time.time()
            t.append(start-end)
            index.add(centroids)
    ...
    def retrieve_from_bank(self, queries, memory_bank):
    ...
    def retrieve(self, queries):
        ...
        print((self.k)//self.chunk_size)
        indexs = [self.index_list[i].search(queries[:, i, :].contiguous(), (self.k)//self.chunk_size)[1] for i in range(self.num_heads)]
        print(indexs[0].shape)
        return self.retrieve_from_bank(queries, [])