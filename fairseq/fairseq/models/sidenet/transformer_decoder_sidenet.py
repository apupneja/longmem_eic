...
from fairseq.modules.dynamic_memory_knn import Hierarchical_Memory
...
class TransformerDecoderSideNetBase(FairseqIncrementalDecoder):
    ...
    def __init__(
        ...
        self.precompute_mem_layer = cfg.precompute_mem_layer
        self.precompute_retrieval = None
        self.previous_qkv_list = []
        ...
        for idx, layer in enumerate(self.layers):
            ...
            if self.external_memory is not None and idx == int(self.retrieval_layer_index / self.layer_reduction_factor):
                ...
                x, layer_attn, knn_index = layer(
                    ...
                    precompute_retrieval=self.precompute_retrieval,
                    knn_config = self.knn_utils,
                )
                if not disable_add_index:
                    self.external_memory.add_index(previous_qkv)#, prev_output_tokens != self.padding_idx)
            elif idx == int(self.retrieval_layer_index / self.layer_reduction_factor - self.precompute_mem_layer):
                ...
                self.precompute_retrieval = layer.precompute_retrieval_output
            else:    
                x, layer_attn, _ = layer(
                    x,
                    enc,
